class Module:
    ...
    def named_parameters(self):
        ...
        for m_name, m_val in self._modules.items():
            for p_name, p_val in m_val.named_parameters().items():
                t.append((f"{m_name}.{p_name}", p_val))
        return t
        """
        dic = self._parameters
        for module_name, module_value in self._modules.items():
            for parameter_name, parameter_value in module_value.named_parameters().items():
                dic[f"{module_name}.{parameter_name}"] = parameter_value
        return dic
        # raise NotImplementedError('Need to implement for Task 0.4')
    ...
    def add_parameter(self, k, v):
        ...
        """
        value = Parameter(v, k)
        self.__dict__["_parameters"][k] = value
        return value
